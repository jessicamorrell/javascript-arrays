...
function reverse(aStr) {
  ...
  nowBackToString = reverseArr.join(" ");
  return nowBackToString;
}


/* THIS IS COOLER (keeps words intact):
function reverse(aStr) {
  var arrString = aStr.split(" ");
  var reverseArrString = [];
  var nowBackToString;
  for (var i = arrString.length-1; i >=0; i--) {
    reverseArrString.push(arrString[i]);
  }
  nowBackToString = reverseArrString.join(" ");
  return nowBackToString;
}
...
function addItem(myGroceryList, itemToAdd) {
    var a = myGroceryList.indexOf(itemToAdd); 
      if (myGroceryList[a] !== itemToAdd) {
...
/*As a continuation of the previous problem, write another function called 'both'.
  ...
  'both' should return a new array full of numbers that only are found in both arr1 and arr2.
*/
